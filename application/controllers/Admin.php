...
class Admin extends CI_Controller
{
    ...
    {
        $data['total_cimb'] = $this->Admin_model->get_total_data('cimb_forms');
        $data['total_bsi'] = $this->Admin_model->get_total_data('bsi_forms');
        $data['total_uob'] = $this->Admin_model->get_total_data('uob_forms');
        $data['total_line'] = $this->Admin_model->get_total_data('line_forms');
        $data['total_bpd'] = $this->Admin_model->get_total_data('bpd_forms');
        // tambahkan untuk tabel lain jika diperlukan
    ...
    public function export_line()
    {
        ...
        // Set header tabel
        $headers = ["No", "Tanggal", "Mitra Code", "Nama DSR", "Nama Nasabah", "No HP Nasabah", "No Rek Nasabah", "Kota Akuisisi", "SS Dashboard", "Foto KTP Nasabah", "SPV", "KOOR", "ASM"];
        $col = 'A';
        foreach ($headers as $header) {
            $sheet->setCellValue($col . '1', $header);
            $col++;
